...
class ScanNetMatchPairDataset(torch.utils.data.Dataset):
  #config :
  # {'trainer': 'HardestContrastiveLossTrainer', 'save_freq_epoch': 1, 'batch_size': 8, 'use_hard_negative': True,
  #  'hard_negative_max_num': 3000, 'num_pos_per_batch': 1024, 'num_hn_samples_per_batch': 256, 'neg_thresh': 1.4,
  #  'pos_thresh': 0.1, 'neg_weight': 1.0, 'use_random_scale': False, 'min_scale': 0.8, 'max_scale': 1.2,
  #  'use_random_rotation': True, 'rotation_range': 360, 'stat_freq': 1, 'lr_update_freq': 250,
  ...
  def __getitem__(self, idx):
    ...
    if self.random_rotation:
      ...
      # generate new rotation matrix to align two view
      trans = T1 @ np.linalg.inv(T0)
      #Word reference system to camera reference system using transformation matrix
    ...
    # Voxelization
    # sparse_quantize create the
    # only one unique coordinate per voxel
